...
{
    const int n = length[0]*length[1];
    const int is_present = acc_is_present(ex, sizeof(FLOAT)*n);
    
#pragma acc kernels if(is_present) present(ex, ey, hz)
...
{
#pragma acc kernels copyin(length[0:2]) present(er, obj, cexly, ceylx, chzlx, chzly)
#pragma acc loop independent
...
{
    const int n = length[0]*length[1];
#pragma acc kernels present(exy, eyx, hzx, hzy)
#pragma acc loop independent
    for (int i=0; i<n; i++) {
        exy[i] = 0.0;
        eyx[i] = 0.0;
...
#pragma acc kernels present(cs, csl)
#pragma acc loop independent
...
{
#pragma acc kernels copyin(length[0:2]) present(obj, er, rer_ex, rer_ey)
#pragma acc loop independent
    for (int j=0; j<length[1]; j++) {
#pragma acc loop independent
        for (int i=0; i<length[0]; i++) {
